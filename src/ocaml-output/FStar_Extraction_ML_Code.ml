...
let _76_32 = (FStar_Util.first_N cg_len ns)
...
let _76_51 = (mlpath_of_mlpath currentModule mlp)
in (match (_76_51) with
| (p, s) -> begin
...
let _76_56 = (mlpath_of_mlpath currentModule mlp)
in (match (_76_56) with
| (p, s) -> begin
(
...
let as_bin_op : FStar_Extraction_ML_Syntax.mlpath  ->  (Prims.string * (Prims.int * fixity) * Prims.string) Prims.option = (fun _76_61 -> (match (_76_61) with
| (ns, x) -> begin
if (is_prims_ns ns) then begin
(FStar_List.tryFind (fun _76_67 -> (match (_76_67) with
| (y, _76_64, _76_66) -> begin
(x = y)
end)) infix_prim_ops)
end else begin
None
...
let as_uni_op : FStar_Extraction_ML_Syntax.mlpath  ->  (Prims.string * Prims.string) Prims.option = (fun _76_71 -> (match (_76_71) with
| (ns, x) -> begin
if (is_prims_ns ns) then begin
(FStar_List.tryFind (fun _76_75 -> (match (_76_75) with
| (y, _76_74) -> begin
(x = y)
end)) prim_uni_ops)
end else begin
...
let as_standard_type = (fun _76_79 -> (match (_76_79) with
| (ns, x) -> begin
if (is_prims_ns ns) then begin
(FStar_List.tryFind (fun _76_83 -> (match (_76_83) with
| (y, _76_82) -> begin
(x = y)
end)) prim_types)
end else begin
...
let as_standard_constructor : FStar_Extraction_ML_Syntax.mlpath  ->  (Prims.string * Prims.string) Prims.option = (fun _76_87 -> (match (_76_87) with
| (ns, x) -> begin
if (is_prims_ns ns) then begin
(FStar_List.tryFind (fun _76_91 -> (match (_76_91) with
| (y, _76_90) -> begin
(x = y)
...
let maybe_paren : ((Prims.int * fixity) * assoc)  ->  (Prims.int * fixity)  ->  FStar_Format.doc  ->  FStar_Format.doc = (fun _76_95 inner doc -> (match (_76_95) with
| (outer, side) -> begin
(
...
let _76_104 = _inner
in (match (_76_104) with
| (pi, fi) -> begin
(

let _76_107 = _outer
in (match (_76_107) with
| (po, fo) -> begin
...
| (_76_131, NonAssoc) -> begin
((pi = po) && (fi = fo))
end
| (_76_135, _76_137) -> begin
false
end))
end))
end)))
...
let escape_or : (FStar_Char.char  ->  Prims.string)  ->  FStar_Char.char  ->  Prims.string = (fun fallback _76_1 -> (match (_76_1) with
| c when (c = '\\') -> begin
"\\\\"
end
| c when (c = ' ') -> begin
...
| FStar_Extraction_ML_Syntax.MLTY_Fun (t1, _76_229, t2) -> begin
(
...
| FStar_Extraction_ML_Syntax.MLE_Var (x, _76_257) -> begin
(FStar_Format.text x)
end
...
let for1 = (fun _76_269 -> (match (_76_269) with
| (name, e) -> begin
(
...
| FStar_Extraction_ML_Syntax.MLE_Let ((rec_, _76_300, lets), body) -> begin
(
...
| (FStar_Extraction_ML_Syntax.MLE_Name (p), ({FStar_Extraction_ML_Syntax.expr = FStar_Extraction_ML_Syntax.MLE_Fun ((_76_333)::[], scrutinee); FStar_Extraction_ML_Syntax.mlty = _76_331; FStar_Extraction_ML_Syntax.loc = _76_329})::({FStar_Extraction_ML_Syntax.expr = FStar_Extraction_ML_Syntax.MLE_Fun (((arg, _76_321))::[], possible_match); FStar_Extraction_ML_Syntax.mlty = _76_318; FStar_Extraction_ML_Syntax.loc = _76_316})::[]) when ((FStar_Extraction_ML_Syntax.string_of_mlpath p) = "FStar.All.try_with") -> begin
(

let branches = (match (possible_match) with
| {FStar_Extraction_ML_Syntax.expr = FStar_Extraction_ML_Syntax.MLE_Match ({FStar_Extraction_ML_Syntax.expr = FStar_Extraction_ML_Syntax.MLE_Var (arg'); FStar_Extraction_ML_Syntax.mlty = _76_348; FStar_Extraction_ML_Syntax.loc = _76_346}, branches); FStar_Extraction_ML_Syntax.mlty = _76_344; FStar_Extraction_ML_Syntax.loc = _76_342} when ((FStar_Extraction_ML_Syntax.idsym arg) = (FStar_Extraction_ML_Syntax.idsym arg')) -> begin
branches
end
| e -> begin
...
| (FStar_Extraction_ML_Syntax.MLE_App ({FStar_Extraction_ML_Syntax.expr = FStar_Extraction_ML_Syntax.MLE_Name (p); FStar_Extraction_ML_Syntax.mlty = _76_367; FStar_Extraction_ML_Syntax.loc = _76_365}, (unitVal)::[]), (e1)::(e2)::[]) when ((is_bin_op p) && (unitVal = FStar_Extraction_ML_Syntax.ml_unit)) -> begin
(doc_of_binop currentModule p e1 e2)
end
| (FStar_Extraction_ML_Syntax.MLE_Name (p), (e1)::[]) when (is_uni_op p) -> begin
(doc_of_uniop currentModule p e1)
end
| (FStar_Extraction_ML_Syntax.MLE_App ({FStar_Extraction_ML_Syntax.expr = FStar_Extraction_ML_Syntax.MLE_Name (p); FStar_Extraction_ML_Syntax.mlty = _76_387; FStar_Extraction_ML_Syntax.loc = _76_385}, (unitVal)::[]), (e1)::[]) when ((is_uni_op p) && (unitVal = FStar_Extraction_ML_Syntax.ml_unit)) -> begin
(doc_of_uniop currentModule p e1)
end
| _76_399 -> begin
(
...
let ids = (FStar_List.map (fun _76_424 -> (match (_76_424) with
| ((x, _76_421), xt) -> begin
...
let _76_472 = (let _172_238 = (as_bin_op p)
in (FStar_Option.get _172_238))
in (match (_76_472) with
| (_76_469, prio, txt) -> begin
(
...
let _76_482 = (let _172_242 = (as_uni_op p)
in (FStar_Option.get _172_242))
in (match (_76_482) with
| (_76_480, txt) -> begin
(
...
let for1 = (fun _76_499 -> (match (_76_499) with
| (name, p) -> begin
...
| (_76_516, (FStar_Extraction_ML_Syntax.MLP_Tuple (_76_518))::[]) -> begin
...
and doc_of_branch : FStar_Extraction_ML_Syntax.mlsymbol  ->  FStar_Extraction_ML_Syntax.mlbranch  ->  FStar_Format.doc = (fun currentModule _76_536 -> (match (_76_536) with
| (p, cond, e) -> begin
(
...
and doc_of_lets : FStar_Extraction_ML_Syntax.mlsymbol  ->  (FStar_Extraction_ML_Syntax.mlletflavor * Prims.bool * FStar_Extraction_ML_Syntax.mllb Prims.list)  ->  FStar_Format.doc = (fun currentModule _76_546 -> (match (_76_546) with
| (rec_, top_level, lets) -> begin
(

let for1 = (fun _76_554 -> (match (_76_554) with
| {FStar_Extraction_ML_Syntax.mllb_name = name; FStar_Extraction_ML_Syntax.mllb_tysc = tys; FStar_Extraction_ML_Syntax.mllb_add_unit = _76_551; FStar_Extraction_ML_Syntax.mllb_def = e; FStar_Extraction_ML_Syntax.print_typ = pt} -> begin
(
...
let ids = (FStar_List.map (fun _76_560 -> (match (_76_560) with
| (x, _76_559) -> begin
(FStar_Format.text x)
end)) ids)
in (
...
and doc_of_loc : FStar_Extraction_ML_Syntax.mlloc  ->  FStar_Format.doc = (fun _76_600 -> (match (_76_600) with
| (lineno, file) -> begin
...
let for1 = (fun _76_611 -> (match (_76_611) with
| (_76_606, x, mangle_opt, tparams, body) -> begin
(
...
let forfield = (fun _76_633 -> (match (_76_633) with
| (name, ty) -> begin
(
...
let forctor = (fun _76_641 -> (match (_76_641) with
| (name, tys) -> begin
(match (tys) with
| [] -> begin
...
| FStar_Extraction_ML_Syntax.MLS_Val (x, (_76_675, ty)) -> begin
(
...
| FStar_Extraction_ML_Syntax.MLM_Let (rec_, _76_704, lets) -> begin
...
in (doc)::((match (x) with
| FStar_Extraction_ML_Syntax.MLM_Loc (_76_717) -> begin
FStar_Format.empty
end
| _76_720 -> begin
...
let rec doc_of_mllib_r : FStar_Extraction_ML_Syntax.mllib  ->  (Prims.string * FStar_Format.doc) Prims.list = (fun _76_723 -> (match (_76_723) with
| FStar_Extraction_ML_Syntax.MLLib (mllib) -> begin
(

let rec for1_sig = (fun _76_730 -> (match (_76_730) with
...
let doc = (FStar_Option.map (fun _76_737 -> (match (_76_737) with
| (s, _76_736) -> begin
(doc_of_sig x s)
end)) sigmod)
in (
...
and for1_mod = (fun istop _76_750 -> (match (_76_750) with
| (x, sigmod, FStar_Extraction_ML_Syntax.MLLib (sub)) -> begin
(
...
let doc = (FStar_Option.map (fun _76_757 -> (match (_76_757) with
| (_76_755, m) -> begin
(doc_of_mod x m)
end)) sigmod)
in (
...
let docs = (FStar_List.map (fun _76_769 -> (match (_76_769) with
| (x, s, m) -> begin
